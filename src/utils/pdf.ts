...
export function generateAvaluoPDF(a: Avaluo){
  ...
  doc.text('Viva Costa Rica', 14, 11)
  doc.setFontSize(7); doc.setTextColor(255,255,255)
  doc.text('Costa Rica · AVALÚOS 20 AÑOS · CFIA IC-11247', 14, 16)
  ...
  addLine('Costo avalúo', `₡${a.costoTotal.toLocaleString('es-CR')} (despl. ₡${a.desplazamientoCosto.toLocaleString('es-CR')} desde Costa Rica) · Pago: ${a.pagoStatus}`)
  ...
  doc.text('Viva Costa Rica · Costa Rica · info@vivacostaricacr.com · Este avalúo virtual es rango estimado con fotos/datos cliente. Para validez bancaria requiere visita física CFIA.', 14, H-10, {maxWidth: W-28, align:'left'})
  doc.text(`Documento generado ${new Date().toLocaleString('es-CR')} · ${a.id}`, 14, H-6)

  doc.save(`${a.id}-vivacostarica-Avaluo.pdf`)
}
...
function genPluses(a: Avaluo): string[] {
  ...
  if(a.direccion.toLowerCase().includes('Costa Rica')) list.push(`Ubicación Costa Rica — alta demanda Occidente, plusvalía estable.`)
